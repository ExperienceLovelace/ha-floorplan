...
export const getRecentWithCache = (
  ...
  const genProm = async () => {
    ...
    const stateHistory = computeHistory(hass, fetchedHistory);
    if (appendingToCache) {
...
const pruneArray = <T extends LineChartState | TimelineState>(
  ...
  arr[updateIndex].last_changed = originalStartTime.getTime();
